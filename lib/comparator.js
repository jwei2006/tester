...
let compareObject = function (src, des, parentName) {
    for(let item in src){
        ...
        try{
            ...
            logger.getLogger().error('%s\.%s 比较错误: %s', parentName, item, e);
        }
    }
};
...
let checkEmpty = function (src, des, parentName) {
    ...
    if(nullSrc || nullDes){
        if(nullSrc != nullDes)
            logger.getLogger().error('%s 比较错误: %s', parentName, '一个为空，另一个非空');
        return false;
    }
    ...
    if(objSrc != objDes){
        logger.getLogger().error('%s 比较错误: %s', parentName, '一个为对象，另一个非对象');
        return false;
    }
    let arraySrc = Array.isArray(src);
    let arrayDes = Array.isArray(des);
    if(arraySrc != arrayDes){
        logger.getLogger().error('%s 比较错误: %s', parentName, '一个为数组，另一个非数组');
        return false;
    }
