...
exports.saveTemp = function (caseName, data) {
    ...
    fs.writeFile(file, data, function(err) {
        if (err) throw err;
        logger.getLogger().debug('存储临时数据%s完成', caseName);
    });
};
...
exports.saveFormal = function () {
    ...
    if(undefined == files || files.size == 0){
        logger.getLogger().warn('没有结果数据，请先执行测试用例，检查数据正确后重试');
        return;
    }
    ...
    for(let file of files){
        ...
        logger.getLogger().debug('保存测试数据： %s', file);
    }
    logger.getLogger().debug('保存本次测试数据完成！');
};
